...
async def lifespan(
    ...
    print(
        f"Загрузка токенизатора LLM: "
        f"{TOKENIZER_MODEL}"
    )

    app.state.llm_tokenizer = (
    ...
    print(
        f"Загрузка sparse-модели: "
        f"{SPARSE_EMBEDDING_MODEL}"
    )

    try:
        app.state.sparse_model = (
            SparseTextEmbedding(
                model_name=
                    SPARSE_EMBEDDING_MODEL,
    ...
    if not app.state.qdrant.collection_exists(
        COLLECTION_NAME
    ):
        raise RuntimeError(
            "Hybrid-коллекция Qdrant "
            f"'{COLLECTION_NAME}' "
            "не найдена"
        )

    print(
        "Wiki.js Hybrid RAG API запущен"
    )
...
def build_user_prompt(
    ...
    context_text = (
        ...
        if contexts
        else "Подходящие источники не найдены."
    )
    ...
    *,
    ...
    if (
        ...
        raise HTTPException(
            status_code=500,
            detail=(
                "Не удалось сформировать prompt "
...
def format_for_openwebui(
    ...
    answer = rewrite_relative_wiki_links(
        answer
    )
